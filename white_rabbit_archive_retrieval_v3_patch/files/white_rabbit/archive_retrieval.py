...
INDEX_VERSION = 3
_INDEX_FILENAME = "archive_search_index_v3.joblib"
...
def _expanded_concepts(index: dict, entities: tuple[str, ...], query: str, *, limit: int = 14) -> tuple[str, ...]:
    """Derive related concepts from archive chunks that directly mention the entity.

    This is corpus-driven query expansion: no Gemini/API credits. It lets an entity
    such as Flock Safety teach the retriever nearby concepts (ALPR, license plate,
    vehicle, cameras, tracking) from the user's own archive.
    """
    if not entities:
        return ()
    ...
    if not seed_idx:
        return ()
    texts = [f"{chunks[i]['title']} {chunks[i]['section']} {chunks[i]['text']}" for i in seed_idx[:40]]
    mat = vectorizer.transform(texts)
    weights = mat.mean(axis=0)
    try:
        arr = np.asarray(weights).reshape(-1)
    except Exception:
        arr = np.asarray(weights.A1)
    ...
    for idx in order:
        if arr[idx] <= 0:
            break
        feat = str(features[idx]).strip().lower()
        ...
        if any(t in entity_words for t in ftoks):
            continue
        if len(ftoks) == 1 and ftoks[0] in _GENERIC_QUERY_TERMS:
            continue
        if any(t in _STOPWORDS for t in ftoks):
            continue
        if feat in seen or len(feat) < 3:
            continue
        # Prefer meaningful bigrams and distinctive single terms.
...
def _concept_score_for_chunk(chunk: dict, concepts: tuple[str, ...]) -> tuple[float, tuple[str, ...]]:
    if not concepts:
        return 0.0, ()
    hay = f"{chunk['title']} {chunk['section']} {chunk['text']}".lower()
    hits = [c for c in concepts if c in hay]
    if not hits:
        return 0.0, ()
    weighted = sum(1.5 if " " in h else 1.0 for h in hits[:8])
    return min(1.0, weighted / 4.0), tuple(hits[:8])
    ...
    for link in registered_links or []:
        ...
        if kind == "internal_article":
            ...
            if url == _canonical_internal_url(article.canonical_url):
                continue
        if (anchor.lower(), url) in seen_links:
            continue
        if kind == "external_research":
...
def retrieve_archive_memory(
    ...
    *,
    ...
    """Entity-aware chunk-level hybrid retrieval across the White Rabbit archive."""
    db_path = Path(db_path)
    ...
    # BM25 uses the non-entity concepts. Exact entity phrases are scored separately,
    # preventing a generic component such as "safety" from acting independently.
    ...
    semantic_query = " ".join([query, *concepts[:10]]) if concepts else query
    semantic = _semantic_scores(index, semantic_query)
    ...
    for i, chunk in enumerate(chunks):
        ...
        concept_score, concept_hits = _concept_score_for_chunk(chunk, concepts)
        title_tokens = set(_tokenize(chunk["title"] + " " + chunk["section"]))
        title_overlap = len((free_term_set or all_query_tokens) & title_tokens) / max(1, len(free_term_set or all_query_tokens))
        anchor_score = _link_anchor_score(chunk, free_term_set or all_query_tokens, phrases)
        raw_sem = float(semantic[i]) if i < len(semantic) else 0.0
        baseline_sem = float(generic_semantic[i]) if i < len(generic_semantic) else 0.0
        sem = raw_sem if direct_entity else max(0.0, raw_sem - 0.72 * baseline_sem)
        lex = float(bm25[i]) if i < len(bm25) else 0.0
        free_overlap = bool(free_term_set & chunk_tokens) if free_term_set else False

        # ENTITY GATE: conceptual results are welcome, generic component-word
        # matches are not. A non-direct result must demonstrate either strong
        # semantic relation or meaningful concept expansion from direct entity hits.
        if entities and not direct_entity:
            if not free_overlap and concept_score < 0.20:
                continue
            # With a named entity present, generic lexical overlap is insufficient.
            # Require either an archive-derived related concept or a genuinely
            # strong semantic match to the entity-enriched query.
            if concept_score < 0.15 and sem < 0.38:
                continue
            if sem < 0.26 and concept_score < 0.25:
                continue
        ...
        if direct_entity:
            ...
            if concept_score >= 0.25 and sem >= 0.25:
        ...
        if score <= 0:
            continue
        if not direct_entity and not matched and not concept_hits and sem < 0.34:
            continue
    ...
    candidate_chunks = scored_chunks[:max(chunk_limit * 8, article_limit * 8, 40)] if chunk_limit > 0 else scored_chunks
    ...
    for url, rows in by_article.items():
        ...
        best = top[0]
        chunk = best["chunk"]
        external = [{"anchor": a, "url": u, "type": "external_research"} for a, u in chunk.get("external_links", ()) if not _is_junk_anchor(a)]
...
def format_archive_memory(memories: list[ArchiveMemory], *, max_source_links_per_article: int = 15) -> str:
    ...
    for memory in memories:
        ...
        out.append(
            f"Connection: {memory.connection_type}\n"
        )
        ...
        out.append(
            ...
            f"external research links={memory.research_source_count}\n"
        )
